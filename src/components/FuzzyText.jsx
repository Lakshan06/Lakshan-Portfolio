...
const FuzzyText = ({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Use the larger of bounding-box and advance-width so bold last-glyphs never clip
      const textBoundingWidth = Math.ceil(
        letterSpacing !== 0
          ? totalWidth
          : Math.max(actualLeft + actualRight, metrics.width)
      );
      const tightHeight = Math.ceil(actualAscent + actualDescent);

      // 50px buffer (was 10) — prevents large bold glyphs from being cut on the right
      const extraWidthBuffer = 50;
